...
export default function ProductRegister() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newProduct = {
      ...
      saleType,
      price: price + '원',
      name: getMorphNames(morphs),
      breeder: '파충류마을', // TODO: 실제 브리더 정보로 대체
      image: '/images/geckos/crested-1.jpg', // TODO: 실제 업로드된 이미지로 대체
    };
      ...
      <main className="pt-16 pb-16">
        <form onSubmit={handleSubmit} className="p-4 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="hidden"
            />
            <p className="mt-2 text-sm text-gray-500">최대 10개까지 업로드 가능</p>
          </div>

          {/* Species & Morph Selection */}
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 text-gray-500 bg-gray-50">판매 정보</span>
            </div>
          </div>
              ...
              {SALE_TYPES.map((option) => (
                ...
                >
                  <input
                    ...
                    name="saleType"
                    value={option.id}
                    checked={saleType === option.id}
                    onChange={() => setSaleType(option.id)}
                    className="sr-only"
                  />
                  <span className="text-sm font-medium">{option.name}</span>
                </label>
          ...
          <div>
            <label htmlFor="price" className="block text-sm font-medium text-gray-700 mb-2">
              {saleType === 'auction' ? '시작가' : '판매가'} <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              <input
                type="text"
          ...
          {/* Submit Button */}
          <div className="sticky bottom-0 -mx-4 -mb-6 p-4 bg-white border-t border-gray-200">
            <button
              type="submit"
              className="w-full py-3 px-4 rounded-lg bg-indigo-600 text-white font-medium hover:bg-indigo-700"
            >
              등록하기
            </button>
          </div>
        ...
        {isBottomSheetOpen && (
          <>
            ...
            />
            <div 
              className={`fixed inset-x-0 bottom-0 bg-white rounded-t-2xl shadow-lg transform transition-transform duration-300 ease-in-out z-[61] ${
